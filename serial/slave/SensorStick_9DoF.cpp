...
int SensorStick_9DoF::setZero(char sensor,char axis,double zero[]){
  ...
  switch (sensor) {
  case ACC: 
    return _accZero[i] =zero[i]; 
    break;
  case GYR: 
    return _gyroZero[i] =zero[i]; 
    break;
  case MAG: 
    return _magZero[i] =zero[i]; 
    break;
  }
}
...
double SensorStick_9DoF::get(char sensor,char axis){
  ...
  switch (sensor) {
  case ACC: 
    return (double)_acc[i]/accSensityivity[i]; 
    break;
  case GYR: 
    return (double)_gyro[i]/gyroSensityivity[i]; 
    break;
  case MAG: 
    return (double)_mag[i]/magSensityivity[i];
    ; 
    break;
  }
}
...
int SensorStick_9DoF::receiveGyro(){
  byte buf[8];
  if(twiRead(ITG3200_ADDRESS, ITG3200_TEMP_OUT, buf, 8) != 0){
    return -1;
  }
...
int SensorStick_9DoF::receiveAcc(){
  byte buf[6];
  if(twiRead(ADXL345_ADDRESS, ADXL345_DATAX0, buf, 6) != 0){
    return -1;
  }
...
int SensorStick_9DoF::receiveMag(){
  byte buf[6];
  if(twiRead(HMC5883L_ADDRESS, HMC5883L_DATA_REGISTER_BEGIN, buf, 6) != 0){
    return -1;
  }
...
int SensorStick_9DoF::gyroRead(int gyro[]){
  byte buf[6];
  if(twiRead(ITG3200_ADDRESS, ITG3200_GYRO_XOUT, buf, 6) != 0){
    return -1;
  }
